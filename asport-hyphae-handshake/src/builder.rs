...
use crate::{
    buffer::Buffer,
    crypto::{
        noise::x25519::{PublicKey, SecretKey},
        CryptoError, SecretKeySetup,
    },
...
impl<'a, T: PayloadDriver + Clone> HandshakeBuilder<'a, T> {
    ...
    pub fn with_cloned_payload_driver<TT: PayloadDriver + Clone>(
        self,
        payload_driver: TT,
    ) -> HandshakeBuilder<'a, TT> {
        HandshakeBuilder {
            payload_driver,
...
impl<T: PayloadDriver + Clone> HandshakeConfig for BasicHandshakeConfig<T> {
    ...
    fn new_initiator(
        ...
        noise_handshake.initialize(
            ...
            self.s
                .as_ref()
                .map(SecretKey::as_bytes)
                .map(SecretKeySetup::from),
            self.rs.as_ref().map(PublicKey::as_ref),
        )?;
