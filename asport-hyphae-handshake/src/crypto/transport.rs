...
use crate::handshake::{
    HandshakeVersion, HYPHAE_INITIAL_SECRET_HKDF_LABEL, HYPHAE_RETRY_SECRET_HKDF_LABEL,
};

use super::{
    backends::{AeadBackend, HashBackend, HashExt},
    CryptoError, InitialCrypto, SymmetricKey, TransportCrypto,
};
...
impl<A: AeadBackend, H: HashBackend> TransportCrypto for BuiltinTransportCrypto<A, H> {
    ...
    fn derive_subkey(
        &self,
        level_secret: &super::SymmetricKey,
        label: &[u8],
        output: &mut super::SymmetricKey,
    ) {
    ...
    fn encrypt_in_place(
        &self,
        packet_key: &super::SymmetricKey,
        packet_id: u64,
        ad: &[u8],
        buffer: &mut [u8],
    ) -> Result<(), CryptoError> {
        self.aead_impl
            .encrypt_in_place(packet_key, packet_id, ad, buffer)
    }
    ...
    fn decrypt_in_place<'a>(
        ...
        self.aead_impl
            .decrypt_in_place(packet_key, packet_id, ad, buffer)
    }
    ...
    fn header_protection_mask(
        ...
        self.aead_impl
            .header_protection_mask(header_key, sample, mask)
    }
}
...
impl<A: AeadBackend, H: HashBackend> InitialCrypto for BuiltinTransportCrypto<A, H> {
    fn initial_level_secret(
        ...
        client_dcid_secret(
            ...
            transport_label,
            client_dcid,
            HYPHAE_INITIAL_SECRET_HKDF_LABEL,
            level_secret,
        )
    }
    ...
    fn retry_tag_secret(
        ...
        client_dcid_secret(
            ...
            transport_label,
            client_dcid,
            HYPHAE_RETRY_SECRET_HKDF_LABEL,
            level_secret,
        )
    }
}

fn client_dcid_secret(
    hash_impl: &impl HashBackend,
    handshake_version: HandshakeVersion,
    transport_label: &[u8],
    client_dcid: &[u8],
    label: &[u8],
    ...
    let ikm = once(handshake_version.label())
...
fn derive_subkey(
    ...
    label: &[u8],
    output: &mut SymmetricKey,
) {
    hash_impl.hkdf(
        &hash_impl.zeros(),
        once(output.as_mut().as_mut_slice()),
        once(level_secret.as_ref().as_slice()),
        label,
    );
}
