...
pub use self::{
    client_hello::{ClientHello, KeyingMaterialExporter},
    connect::Connect,
    ...
    B: AsRef<[u8]>,
{
    ...
    /// Receives an `ClientHello`
    pub fn recv_client_hello(&self, header: ClientHelloHeader) -> ClientHello<side::Rx> {
        let (uuid, token, forward_mode, expected_port_range) = header.into();
        ClientHello::<side::Rx>::new(uuid, token, forward_mode, expected_port_range)
    }

    /// Sends a `ServerHello`
    pub fn send_server_hello(&self, server_hello: ServerHelloHeader) -> ServerHello<side::Tx> {
        ServerHello::<side::Tx>::new(server_hello)
    }

    /// Receives a `ServerHello`
    pub fn recv_server_hello(&self, header: ServerHelloHeader) -> ServerHello<side::Rx> {
        let (handshake_code, port) = header.into();
        ServerHello::<side::Rx>::new(handshake_code, port)
    }
