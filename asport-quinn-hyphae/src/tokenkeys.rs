...
use hyphae_handshake::crypto::{
    SymmetricKey, SyncCryptoBackend, TransportCrypto, HYPHAE_AEAD_TAG_LEN,
};
...
impl<B: SyncCryptoBackend> HandshakeTokenKey for HyphaeHandshakeTokenKey<B> {
    fn aead_from_hkdf(&self, random_bytes: &[u8]) -> Box<dyn AeadKey> {
        ...
        self.crypto
            .hkdf(&self.key, random_bytes, b"", &mut aead_key_hash);
        let mut aead_key = SymmetricKey::default();
        aead_key
            .as_mut()
            .copy_from_slice(&self.crypto.hash_as_slice(&aead_key_hash)[0..SymmetricKey::SIZE]);
        Box::new(HyphaeAeadKey::<B> {
...
impl<B: SyncCryptoBackend> AeadKey for HyphaeAeadKey<B> {
    fn seal(
        ...
        self.crypto
            .encrypt_in_place(&self.key, 0, additional_data, data.as_mut_slice())
    ...
    fn open<'a>(
        ...
        let len = self
            .crypto
            .decrypt_in_place(&self.key, 0, additional_data, data)
            .map_err(|_| quinn_proto::crypto::CryptoError)?
            .len();
        Ok(&mut data[0..len])
    }
