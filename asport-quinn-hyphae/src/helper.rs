...
use crate::{
    config::HyphaeCryptoConfig, customization::QuinnHandshakeData, HYPHAE_H_V1_QUIC_V1_VERSION,
};
...
{
    let mut endpoint =
        hyphae_server_endpoint(crypto_config.clone(), transport_config.clone(), socket)?;
    ...
    B: SyncCryptoBackend,
{
    let mut endpoint = Endpoint::new(
        hyphae_endpoint_config(default_client_crypto.crypto_backend()),
        None,
        socket,
        quinn_runtime()?,
    )?;
...
/// `client_crypto`.
pub fn hyphae_client_config<C, B>(
    client_crypto: Arc<HyphaeCryptoConfig<C, B>>,
) -> quinn::ClientConfig
where
    C: SyncHandshakeConfig,
    C::Driver: QuinnHandshakeData,
...
/// `server_crypto`.
pub fn hyphae_server_config<C, B>(
    server_crypto: Arc<HyphaeCryptoConfig<C, B>>,
) -> quinn::ServerConfig
where
    C: SyncHandshakeConfig,
...
fn quinn_runtime() -> io::Result<Arc<dyn quinn::Runtime>> {
    quinn::default_runtime().ok_or(io::Error::new(
        io::ErrorKind::Other,
