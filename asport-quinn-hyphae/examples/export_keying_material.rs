...
use quinn_hyphae::{
    ...
    HandshakeBuilder, RustCryptoBackend,
};
use rand_core::OsRng;
...
async fn main() -> Result<()> {
    ...
    let server_crypto_config = HandshakeBuilder::new("Noise_XX_25519_ChaChaPoly_BLAKE2s")
    ...
    let client_crypto_config = HandshakeBuilder::new("Noise_XX_25519_ChaChaPoly_BLAKE2s")
    ...
    let server_task = tokio::spawn(async move {
        if let Some(incoming) = server_endpoint.accept().await {
            ...
            println!(
                "Server: App authentication key: {:02x?}",
                &server_auth_key[..8]
            );
            ...
            println!(
                "Server: Proof-of-possession key: {:02x?}",
                &server_pop_key[..8]
            );
    ...
    client_conn
        .export_keying_material(&mut client_auth_key, b"app-auth-v1", b"")
        .map_err(|e| anyhow::anyhow!("Client auth key export failed: {:?}", e))?;
    println!(
        "Client: App authentication key: {:02x?}",
        &client_auth_key[..8]
    );
    ...
    client_conn
        .export_keying_material(&mut client_metadata_key, b"metadata-encryption", b"")
    ...
    client_conn
        .export_keying_material(&mut client_pop_key, b"proof-of-possession", session_context)
        .map_err(|e| anyhow::anyhow!("Client PoP key export failed: {:?}", e))?;
    println!(
        "Client: Proof-of-possession key: {:02x?}",
        &client_pop_key[..8]
    );

    // Wait for server to complete and get server keys
    let (server_auth_key, server_channel_binding, server_metadata_key, server_pop_key) =
        server_task.await??;
    ...
    println!(
        ...
        client_channel_binding == server_channel_binding
    );
    println!(
        "Metadata keys match: {}",
        client_metadata_key == server_metadata_key
    );
    ...
    client_conn
        .export_keying_material(&mut key1, b"label1", b"")
        .map_err(|e| anyhow::anyhow!("Key1 export failed: {:?}", e))?;
    client_conn
        .export_keying_material(&mut key2, b"label2", b"")
    ...
    client_conn
        .export_keying_material(&mut ctx_key1, b"same-label", b"context1")
        .map_err(|e| anyhow::anyhow!("Context key1 export failed: {:?}", e))?;
    client_conn
        .export_keying_material(&mut ctx_key2, b"same-label", b"context2")
